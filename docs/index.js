...
return false;}
function getRoute(x,y,direction,n){let tmpRoutes=getNeighborRoutes(x,y);if(!isPassableNeighbor(x,y,tmpRoutes)){return false;}
if(direction==1&&0<=x-n){let passable=true;for(let i=1;i<=n;i++){tmpRoutes=getNeighborRoutes(x-i,y);if(!isPassableRoute(x-i,y,tmpRoutes)){passable=false;break;}}
if(passable)return[x-1,y,1];}
if(direction==2&&x+n<size){let passable=true;for(let i=1;i<=n;i++){tmpRoutes=getNeighborRoutes(x+i,y);if(!isPassableRoute(x+i,y,tmpRoutes)){passable=false;break;}}
if(passable)return[x+1,y,2];}
if(direction==3&&0<=y-n){let passable=true;for(let i=1;i<=n;i++){tmpRoutes=getNeighborRoutes(x,y-i);if(!isPassableRoute(x,y-i,tmpRoutes)){passable=false;break;}}
if(passable)return[x,y-1,3];}
if(direction==4&&y+n<size){let passable=true;for(let i=1;i<=n;i++){tmpRoutes=getNeighborRoutes(x,y+i);if(!isPassableRoute(x,y+i,tmpRoutes)){passable=false;break;}}
...
function generateSiritoriCandidates(){const candidates={};for(const[aiueo,idioms]of Object.entries(siritoriList)){if(candidates[aiueo]){const newIdioms=candidates[aiueo].concat(idioms);candidates[aiueo]=newIdioms;}else{candidates[aiueo]=idioms;}}
return candidates;}
function getIdioms(){const siritori=generateSiritoriCandidates();const aiueos=Object.keys(siritori);let aiueo=aiueos[getRandomInt(0,aiueos.length)];let list=[];let generating=true;while(generating){for(let i=0;i<30;i++){const tmpIdioms=siritori[aiueo];if(tmpIdioms){let dupricated=true;for(let i=0;i<5;i++){const idiom=tmpIdioms[getRandomInt(0,tmpIdioms.length)];aiueo=idiom[idiom.length-1];if(!list.includes(idiom)){list.push(idiom);dupricated=false;break;}}
