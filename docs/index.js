const remSize=parseInt(getComputedStyle(document.documentElement).fontSize);const tmpCanvas=document.createElement('canvas');let siritoriList;let size=10;let meiro=new Array(12);let score=0;let counter=0;let processed;let idioms;const words='アイウエオカキクケコサシスセソタチツテトナニヌネノハヒフヘホマミムメモヤユヨラリルレロワヲンヴガギグゲゴザジズゼゾダヂヅデドバビブベボパピプペポァィゥェォッャュー'.split('');function loadConfig(){if(localStorage.getItem('darkMode')==1){document.documentElement.dataset.theme='dark';}}
...
if(hinted){break;}}}
if(k==idiom.length-1){j+=1;k=1;}else{k+=1;}}}
...
return false;}
function getRoute(x,y,direction,n){var tmpRoutes=getNeighborRoutes(x,y);if(!isPassableNeighbor(x,y,tmpRoutes)){return false;}
if(direction==1&&0<=x-n){var passable=true;for(var i=1;i<=n;i++){tmpRoutes=getNeighborRoutes(x-i,y);if(!isPassableRoute(x-i,y,tmpRoutes)){passable=false;break;}}
if(passable){return[x-1,y,1];}}
if(direction==2&&x+n<size){var passable=true;for(var i=1;i<=n;i++){tmpRoutes=getNeighborRoutes(x+i,y);if(!isPassableRoute(x+i,y,tmpRoutes)){passable=false;break;}}
if(passable){return[x+1,y,2];}}
if(direction==3&&0<=y-n){var passable=true;for(var i=1;i<=n;i++){tmpRoutes=getNeighborRoutes(x,y-i);if(!isPassableRoute(x,y-i,tmpRoutes)){passable=false;break;}}
if(passable){return[x,y-1,3];}}
if(direction==4&&y+n<size){var passable=true;for(var i=1;i<=n;i++){tmpRoutes=getNeighborRoutes(x,y+i);if(!isPassableRoute(x,y+i,tmpRoutes)){passable=false;break;}}
...
if(k==idioms[j].length-1){j+=1;k=1;}else{k+=1;}}}
...
function generateSiritoriCandidates(){var candidates={};for(var arr of Object.entries(siritoriList)){var aiueo=arr[0];var idioms=arr[1];if(candidates[aiueo]){var newIdioms=candidates[aiueo].concat(idioms);candidates[aiueo]=newIdioms;}else{candidates[aiueo]=idioms;}}
return candidates;}
function getIdioms(){var siritori=generateSiritoriCandidates();var aiueos=Object.keys(siritori);var aiueo=aiueos[getRandomInt(0,aiueos.length)];var list=[];var generating=true;while(generating){for(var i=0;i<30;i++){var tmpIdioms=siritori[aiueo];if(tmpIdioms){var dupricated=true;for(var i=0;i<5;i++){var idiom=tmpIdioms[getRandomInt(0,tmpIdioms.length)];aiueo=idiom[idiom.length-1];if(!list.includes(idiom)){list.push(idiom);dupricated=false;break;}}
if(dupricated){break;}}else{break;}}
if(list.length==30){generating=false;}else{aiueo=aiueos[getRandomInt(0,aiueos.length)];list=[];}}
return list;}
var timerText=document.getElementById('meiro');resizeFontSize(timerText);window.addEventListener('resize',function(){resizeFontSize(timerText);});fetch('/kana-siritori/index.json').then(response=>response.json()).then(data=>{siritoriList=data;idioms=getIdioms();generateGame();while(solvedPanel.firstChild){solvedPanel.removeChild(solvedPanel.firstChild);}
showAnswer();});